...
def cost_mat(X,Y,N,M):
    XX = tf.reduce_sum(input_tensor=tf.multiply(X,X),axis=1)
    YY = tf.reduce_sum(input_tensor=tf.multiply(Y,Y),axis=1)
    C1 = tf.transpose(a=tf.reshape(tf.tile(XX,[M]),[M,N]))
    C2 = tf.reshape(tf.tile(YY,[N]),[N,M])
    C3 = tf.transpose(a=tf.matmul(Y,tf.transpose(a=X)))
    C = C1 + C2 - 2*C3;
    return C

def K_tild(u,v,C,N,M,epsilon):
    C_tild = C - tf.transpose(a=tf.reshape(tf.tile(u[:,0],[M]),[M,N])) - tf.reshape(tf.tile(v[:,0],[N]),[N,M])
...
def sinkhorn_step_log(j,u,v,C, N,M,epsilon,Lambda = 1):
    ...
    Ku = tf.reshape( tf.reduce_sum(input_tensor=K_tild(u,v,C,N,M,epsilon),axis = 1) ,[N,1] )
    u = Lambda * ( epsilon*(tf.math.log(mu) - tf.math.log(Ku +10**(-6))) + u )
    Kv = tf.reshape( tf.reduce_sum(input_tensor=K_tild(u,v,C,N,M,epsilon),axis = 0), [M,1] )
    v = Lambda * ( epsilon*(tf.math.log(nu) - tf.math.log(Kv +10**(-6))) + v )
    j += 1
...
def sinkhorn_loss(X,Y):
    ...
    N = tf.shape(input=X)[0] # sample size from mu_theta
    M = tf.shape(input=Y)[0] # sample size from \hat nu
    D = tf.shape(input=Y)[1] # dimension of the obervation space
    C = cost_mat(X,Y,N,M)
    K = tf.exp(-C/epsilon)
    #sinkhorn iterations
    j0 = tf.constant(0)
    ...
    cond=cond_iter, body=sinkhorn_step_log, loop_vars=[j0, u0, v0,C, N,M,epsilon])
    gamma_log = K_tild(u,v,C,N,M,epsilon)
    final_cost = tf.reduce_sum(input_tensor=gamma_log*C)
    return final_cost
...
class VariationalAutoencoder(object):
    ...
    def __init__(self, network_architecture, transfer_fct=tf.nn.softplus, 
        ...
        # tf Graph input
        self.x = tf.compat.v1.placeholder(tf.float32, [batch_size, network_architecture["n_input"]])
    ...
    def _create_network(self):
        ...
        self.z = tf.random.uniform((self.batch_size, n_z), dtype=tf.float32)
    ...
    def generate(self, z_sample):
        ...
        zz = tf.compat.v1.placeholder(tf.float32, [1, network_architecture["n_z"]])
